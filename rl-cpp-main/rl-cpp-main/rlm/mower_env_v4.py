...
class MowerEnv(gym.Env):
    ...
    def __init__(
            ...
            verbose=False,
            metrics_dir=None,
            sdf_reward_scale=1.0,  # Add a parameter for SDF reward scale
    ):
        ...
        self.sdf_reward_scale = sdf_reward_scale  # Initialize the SDF reward scale
        self.sdf_map = None
        # Derived parameters
    ...
    def reset(self):
        ...
        max_distance = self.lidar_range / self.meters_per_pixel
        self.sdf_map = np.full_like(self.coverage_map, fill_value=max_distance, dtype=np.float32)
        self.global_sdf_map = np.full_like(self.coverage_map, fill_value=max_distance, dtype=np.float32)
        
        # 初始化已知区域掩码
        self.known_area_mask = np.zeros_like(self.coverage_map, dtype=bool)
        
        # 初始化 SDF 地图
        self._update_sdf_maps()
        
        # 初始化 prev_zero_sdf_ratio (只考虑已知区域)
        known_pixels = np.sum(self.known_area_mask)
        if known_pixels > 0:
            zero_sdf_pixels = np.sum((self.sdf_map == 0) & self.known_area_mask)
            self.prev_zero_sdf_ratio = zero_sdf_pixels / known_pixels
        else:
            self.prev_zero_sdf_ratio = 0
    ...
    def step(self, action):
        ...
        self.noisy_heading = np.random.normal(
            loc=self.heading, scale=self.heading_noise) % (2 * math.pi)#加噪处理
        if not collided:
        ...
        self.local_sdf_old = self.sdf_map[i1:i2, j1:j2].copy()
        # 计算激光雷达点云数据和观测结果，并更新环境中的障碍物信息
...
#===========================SDF_reward===========================
        self._update_sdf_maps()
        zero_sdf_pixels = np.sum((abs(self.sdf_map) <= 1) & self.known_area_mask)
        total_pixels = np.sum(self.known_area_mask)
        zero_sdf_ratio = zero_sdf_pixels / total_pixels if total_pixels > 0 else 0

        # 0值边界奖励
        delta_zero_sdf_ratio = zero_sdf_ratio - self.prev_zero_sdf_ratio
        self.prev_zero_sdf_ratio = zero_sdf_ratio
        reward_sdf_zero = - (0.7 * zero_sdf_ratio + 0.7 * delta_zero_sdf_ratio)

        #之前是使用已覆盖区域sdf地图的0值比例作为奖励，但是这样会导致机器人只在已覆盖区域移动，
        #  所以改为使用当前位置的sdf值惩罚，如果是负数就惩罚，正数就0
        # position_p_int = np.clip(self.position_p.astype(int), 
        #                 [0, 0], 
        #                 [self.sdf_map.shape[1]-1, self.sdf_map.shape[0]-1])
        # current_sdf = self.sdf_map[position_p_int[1], position_p_int[0]]

        # reward_sdf_zero = min(0, current_sdf) # Only apply negative values as penalty

        position_p_int = np.clip(self.position_p.astype(int), 
                                [0, 0], 
                                [self.sdf_map.shape[1]-1, self.sdf_map.shape[0]-1])

        coverage_sdf = self.sdf_map[position_p_int[1], position_p_int[0]]
        obstacle_sdf = self.global_sdf_map[position_p_int[1], position_p_int[0]]
        # 设置参数
        epsilon = 1e-6  # 避免除0
        distance_threshold = self.mower_radius*2   # 设置有效距离阈值

        if 0 < obstacle_sdf < distance_threshold:
            safe_distance = self.mower_radius * 1
            if obstacle_sdf > safe_distance:
                obstacle_reward = self.sdf_reward_scale / (obstacle_sdf + epsilon)
            else:
                # 太近时给予负奖励
                obstacle_reward = -self.sdf_reward_scale * (safe_distance - obstacle_sdf)
        else:
            obstacle_reward = 0


        # 合并奖励
        sdf_reward = 0.05*obstacle_reward  + reward_sdf_zero
        ...
        if self.constant_reward_always or newly_visited_sum == 0:
            ...
            reward_const = 0
        #reward = reward_area + reward_ovrlp + reward_tv + reward_frontier + reward_turn + reward_coll + reward_goal + reward_trunc + reward_const + sdf_reward
        reward = reward_area + reward_ovrlp +sdf_reward+ reward_frontier + reward_turn + reward_coll + reward_goal + reward_trunc + reward_const
        # Log metrics
    ...
    def _compute_lidar_pts(self, position_m, heading):
        ...
        Parameters
        ----------
        position_m : 激光雷达传感器的位置（以米为单位）
        heading : 激光雷达传感器的朝向（以弧度为单位

        Returns
        -------
        lidar_pts : 检测到的2D点的绝对像素坐标数组。
    ...
    def _compute_sdf(self, binary_map):

        dist_outside = distance_transform_edt(1 - binary_map)
        dist_inside = distance_transform_edt(binary_map)
        # 将障碍物内部距离设为负值
        sdf_map = np.where(binary_map, -dist_inside, dist_outside)
        #num_small_sdf = np.sum(np.abs(sdf_map) <= 1)
        
        return sdf_map
    def _update_sdf_maps(self):
        """更新两个 SDF 地图：
        1. coverage_sdf_map: 已覆盖区域的 SDF
        2. obstacle_sdf_map: 障碍物的 SDF
        """
        # 更新已知区域掩码
        self.known_area_mask = self.coverage_map > 0
        
        # 1. 计算覆盖区域的 SDF
        # coverage_binary = self.coverage_map.copy()
        # coverage_binary[~self.known_area_mask] = 0
        # self.sdf_map = self._compute_sdf(coverage_binary)
        coverage_binary = (self.coverage_map > 0).astype(np.uint8)
        self.sdf_map = self._compute_sdf(coverage_binary)
        # 2. 计算障碍物的 SDF
        obstacle_binary = np.zeros_like(self.coverage_map)
        obstacle_binary[self.known_obstacle_map > 0] = 1
        obstacle_binary[self.unknown_obstacle_map > 0] = 1
        self.global_sdf_map = self._compute_sdf(obstacle_binary)
        
        # 处理未知区域
        max_distance = self.lidar_range / self.meters_per_pixel 
        self.sdf_map[~self.known_area_mask] = max_distance
        self.global_sdf_map[~self.known_area_mask] = max_distance   
#====================================v2===============================
    # def _update_sdf_map(self):
    #     binary_map = (self.coverage_map > 0).astype(np.uint8)
    #     self.sdf_map = self._compute_sdf(binary_map)
#====================================v1===============================
    # def _update_sdf_map(self,old_position_m, position_m, radius_m):
    #     radius_m = max(self.mower_radius, self.lidar_range)
    #     i1, i2, j1, j2 = self._get_local_neighborhood_indices(
    #         old_position_m, position_m, radius_m)
    #     local_coverage = self.coverage_map[i1:i2, j1:j2].copy()
    #     local_sdf = self._compute_sdf(local_coverage)
    #     self.sdf_map[i1:i2, j1:j2] = local_sdf
#====================================v3===============================
    # def _update_sdf_map(self):
    # # 将未知区域设为障碍物，以避免计算到未知区域的距离
    #     known_coverage_map = self.coverage_map.copy()
    #     known_coverage_map[self.unknown_obstacle_map == 1] = 0  # 将未知障碍物区域设为0
    #     known_coverage_map[self.known_obstacle_map == 1] = 0    # 将已知障碍物区域设为0
    #     # 计算全局SDF
    #     self.sdf_map = self._compute_sdf(known_coverage_map)
