...
class MowerEnv(gym.Env):
    ...
    def __init__(
        ...
        initial_position=None,  # Add a parameter for SDF reward scale
    ):
...
#===========================SDF_reward===========================
        ...
        if not collided:
            ...
            local_bond_sdf = self.obstacle_sdf_map[i1:i2, j1:j2]
            local_bond_sdf = - local_bond_sdf + 5.12
            local_bond_sdf[local_bond_sdf < 1] = 1
            local_bond_sdf[local_bond_sdf > 5.12] = 0

            obstacle_reward = self.newly_visited_reward_scale * (newly_visited_dilated * local_bond_sdf).sum()
            obstacle_reward = obstacle_reward / max_newly_visited_sum
            obstacle_reward = min(obstacle_reward, self.newly_visited_reward_max)

        sdf_reward = obstacle_reward  + reward_sdf_zero
        ...
        reward = reward_ovrlp + sdf_reward+ reward_frontier + reward_turn + reward_coll + reward_goal + reward_trunc + reward_const
        # Log metrics
