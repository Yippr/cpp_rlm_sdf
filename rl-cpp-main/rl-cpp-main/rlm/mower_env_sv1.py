...
class MowerEnv(gym.Env):
    ...
    def __init__(
        ...
        verbose = False,
        metrics_dir = None,
        sdf_reward_scale=1.0, 
        initial_position=None,  # Add a parameter for SDF reward scal
    ):
    ...
    def reset(self):
        ...
        # Randomize initial pose or use fixed initial position
        if self.initial_position is not None:
            self.position_m = np.array(self.initial_position)
            self.position_p = self.position_m * self.pixels_per_meter
            self.noisy_position_m = self.position_m  # 去除位置噪声
            self.noisy_position_p = self.position_p  # 去除位置噪声
            self.heading = 0  # 固定朝向
            self.noisy_heading = self.heading  # 去除朝向噪声
        else:
        ...
        self.prev_zero_sdf_pixels = np.sum(self.coverage_sdf_map == 1)
...
#===========================SDF_reward===========================
        ...
        reward_sdf_zero = 0
        zero_sdf_pixels = np.sum(self.coverage_sdf_map == 1)
        if not collided:
            zero_sdf_ratio = zero_sdf_pixels / math.sqrt(self.coverage_in_pixels) if self.coverage_in_pixels > 0 else 0
            zero_sdf_ratio /= 4
            zero_sdf_ratio *= self.global_tv_reward_scale
            zero_sdf_ratio = np.sign(zero_sdf_ratio) * min(abs(zero_sdf_ratio), self.global_tv_reward_max)

            delta_zero_sdf_ratio = zero_sdf_pixels - self.prev_zero_sdf_pixels
            delta_zero_sdf_ratio *= self.meters_per_pixel / self.step_size / self.max_lin_vel
            delta_zero_sdf_ratio /= 2.5
            delta_zero_sdf_ratio *= self.local_tv_reward_scale
            delta_zero_sdf_ratio = np.sign(delta_zero_sdf_ratio) * min(abs(delta_zero_sdf_ratio), self.local_tv_reward_max)

            reward_sdf_zero = - (zero_sdf_ratio + delta_zero_sdf_ratio)
            # print("------------------")
            # print(zero_sdf_pixels - self.prev_zero_sdf_pixels)
            # print(total_variation_diff)
            # # A template of data generate...
            # self.xdata = np.append(self.xdata, zero_sdf_pixels - self.prev_zero_sdf_pixels)
            # self.ydata = np.append(self.ydata, total_variation_diff)
            # # update x, y data
        ...
        self.prev_zero_sdf_pixels = zero_sdf_pixels
        

        position_p_int = np.clip(self.position_p.astype(int), 
                                [0, 0], 
                                [self.coverage_sdf_map.shape[1]-1, self.coverage_sdf_map.shape[0]-1])

        # coverage_sdf = self.coverage_sdf_map[position_p_int[1], position_p_int[0]]
        # obstacle_sdf = self.obstacle_sdf_map[position_p_int[1], position_p_int[0]]
        # # 设置参数
        # epsilon = 1e-6  # 避免除0
    ...
    def _initialize_maps(self):
        ...
        self.coverage_sdf_map = self.coverage_map.copy()
        self.obstacle_sdf_map = self.coverage_map.copy()
    ...
    def _update_sdf_maps(self):
        ...
        self.coverage_sdf_map = self._compute_sdf(coverage_binary)
        # 2. 计算障碍物的 SDF
        obstacle_binary = np.zeros_like(self.coverage_map)
        obstacle_binary[self.known_obstacle_map > 0] = 1
        self.obstacle_sdf_map = self._compute_sdf(obstacle_binary)
